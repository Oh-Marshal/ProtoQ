...
//
// messageID: 消息类型标识（使用 PacketData.Opcode 的低字节，对标 uni-protocol 的 1 字节 messageId）
...
func (d *MessageDispatcher) Dispatch(conn api.Connection, frame *api.PacketData) (*MessageContext, error) {
	...
	completed := api.FilterChainCompleted(func(ctx api.Context) error {
		...
		if handler == nil {
			return fmt.Errorf("protoq/basic/dispatcher: 未注册的消息处理器：messageID=0x%02X", messageID)
		}

		result, err := handler(ctx)
		if err != nil {
			return fmt.Errorf("protoq/basic/dispatcher: Handler 执行失败 (messageID=0x%02X): %w", messageID, err)
		}
...
func (d *EventDispatcher) Dispatch(conn api.Connection, event string, message interface{}) error {
	...
	if handler == nil {
		return fmt.Errorf("protoq/basic/dispatcher: 未注册的事件处理器：event=%s", event)
	}
