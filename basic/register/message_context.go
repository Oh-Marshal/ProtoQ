...
// conn: 本次请求所属的连接
// frame: 解码后的请求帧
func NewMessageContext(conn api.Connection, frame *api.PacketData) *MessageContext {
...
// PacketData 返回当前请求的解码后协议帧。
//
// 对标 uni-protocol Context.getPacketData()。
// Go 中 PacketData 对应 api.PacketData 结构体。
func (c *MessageContext) PacketData() *api.PacketData {
	return c.frame
}
...
// 返回值可以是：nil（无响应）、*PacketData（直接帧）、[]byte（二进制体）、
