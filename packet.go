...
// NewRequestPacket 创建一个请求帧。
// opcode: 操作码
// seq: 序列号（仅当需要应答时有效）
// body: 消息体
// requiresAck: 是否需要应答
// crc: 是否附加 CRC
func NewRequestPacket(opcode uint32, seq uint32, body []byte, requiresAck bool, crc bool) *PacketData {
	f := &PacketData{
		Flags:  0,
		Opcode: opcode,
		Seq:    seq,
...
// NewResponsePacket 创建一个响应帧。
// 响应帧自动设置 DIR=1，ACK_REQ=0，并原样带回请求的 Seq。
func NewResponsePacket(opcode uint32, seq uint32, body []byte, requestFlags Flags) *PacketData {
	f := &PacketData{
		Flags:  0,
		Opcode: opcode,
		Seq:    seq,
...
// NewNotificationPacket 创建一个通知帧（无需应答的单向消息）。
func NewNotificationPacket(opcode uint32, body []byte, crc bool) *PacketData {
	f := &PacketData{
		Flags:  0,
		Opcode: opcode,
		Seq:    0,
...
// IsRequest 返回是否为请求帧。
func (f *PacketData) IsRequest() bool { return f.Flags.IsRequest() }

// IsResponse 返回是否为响应帧。
func (f *PacketData) IsResponse() bool { return f.Flags.IsResponse() }

// RequiresAck 返回是否需要应答。
func (f *PacketData) RequiresAck() bool { return f.Flags.RequiresAck() }

// String 返回帧的简要描述。
func (f *PacketData) String() string {
	dir := "REQ"
	if f.IsResponse() {
		dir = "RESP"
	}
...
func (f *PacketData) FrameSize() int {
...
func (f *PacketData) HeaderSize() int {
	size := 1 + 1 // Magic + Flags
	if f.Flags.HasBodyLen() {
		size += 2
	}
