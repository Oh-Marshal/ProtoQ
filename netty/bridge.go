...
func NewConnectionBridge(conn *api.Conn, beanReg *BeanRegister) *ConnectionBridge {
	return &ConnectionBridge{
		conn:    conn,
		beanReg: beanReg,
		writeCh: make(chan *api.PacketData, 64),
...
func (b *ConnectionBridge) handleFrame(frame *api.PacketData) {
...
func (b *ConnectionBridge) decryptBody(frame *api.PacketData) error {
	if len(frame.Body) == 0 {
		return nil
	}
...
func (b *ConnectionBridge) writeLoop() {
	for frame := range b.writeCh {
		...
		// 2. 写出帧（Conn.WritePacket 负责编码 + 写入）
		if err := b.conn.WritePacket(frame); err != nil {
			if b.conn.IsClosed() {
				return
			}
			// 写入失败，停止写循环
...
func (b *ConnectionBridge) encryptBody(frame *api.PacketData) error {
	if len(frame.Body) == 0 {
		return nil
	}
...
// WritePacket 向出站写通道提交一帧。
//
// 对标 uni-protocol NettyMessageBridge.write 的对外接口。
// 非阻塞：若写通道已满，可能在 goroutine 中异步写入。
func (b *ConnectionBridge) WritePacket(frame *api.PacketData) {
	select {
...
func (b *ConnectionBridge) handleFatalError(requestFrame *api.PacketData, err error) {
	// 尝试写错误响应帧
...
func (b *ConnectionBridge) writeErrorResponse(requestFrame *api.PacketData, err error) {
	if requestFrame == nil || !requestFrame.RequiresAck() {
		return
	}

	errMsg := []byte(err.Error())
	resp := api.NewResponsePacket(requestFrame.Opcode, requestFrame.Seq, errMsg, requestFrame.Flags)

	// 直接写回（不经过加密，错误响应应为明文）
	b.conn.WritePacket(resp)
}
...
//   - response 是 *api.PacketData → 直接写出
//   - response 是 []byte → 封装为响应帧写出
//   - response 是 nil → 不写回
func (b *ConnectionBridge) writeResponse(requestFrame *api.PacketData, response interface{}) {
	...
	case []byte:
		// []byte → 封装为响应帧
		respFrame := api.NewResponsePacket(requestFrame.Opcode, requestFrame.Seq, resp, requestFrame.Flags)
		b.WritePacket(respFrame)

	case nil:
		// nil 响应 → 不写回（对标 uni-protocol EmptyPayload）
		// 但仍需 WritePacket 空响应以完成 ACK 配对
		// 空 Body 响应帧
		respFrame := api.NewResponsePacket(requestFrame.Opcode, requestFrame.Seq, nil, requestFrame.Flags)
		b.WritePacket(respFrame)
