...
func (r *CodecRegister) Register(codec protoq.Codec) {
	if codec == nil {
		return
	}
	r.mu.Lock()
...
func (r *CodecRegister) Match(encryptType uint16) protoq.Codec {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
//   protoq.Codec      → CodecRegister.Register(codec)
//   protoq.Filter     → FilterChain.AddFilter(filter)
...
type BeanRegister struct {
	...
	mu          sync.RWMutex
	connections map[string]protoq.Connection
}
...
func NewBeanRegister() *BeanRegister {
	return &BeanRegister{
		...
		connections:       make(map[string]protoq.Connection),
	}
}
...
func (r *BeanRegister) RegisterMessageHandler(messageID uint32, handler protoq.MessageHandler) {
...
func (r *BeanRegister) RegisterEventHandler(event string, handler protoq.EventHandler) {
...
func (r *BeanRegister) AddConnection(clientID string, conn protoq.Connection) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *BeanRegister) GetConnection(clientID string) protoq.Connection {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *BeanRegister) GetAllConnections() map[string]protoq.Connection {
	r.mu.RLock()
	defer r.mu.RUnlock()
	snapshot := make(map[string]protoq.Connection, len(r.connections))
