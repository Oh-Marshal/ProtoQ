...
// NegotiateFilter 内容协商过滤器，实现 protoq.Filter 接口。
//
...
func (f *NegotiateFilter) DoFilter(ctx protoq.Context, chain protoq.FilterChain) error {
	frame := ctx.Frame()
	if frame == nil {
		// 无帧数据，直接放行
