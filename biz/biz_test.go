...
func applyBizHandlers(server *api.Server, negotiator biz.Negotiator) {
	if negotiator == nil {
...
func makeTestNegotiateHandler(neg biz.Negotiator) api.ConnHandler {
	var sessionSeq atomic.Uint64
	return func(ctx *api.ConnContext, opcode uint32, body []byte) ([]byte, error) {
...
func makeTestHeartbeatHandler() api.ConnHandler {
	return func(ctx *api.ConnContext, opcode uint32, body []byte) ([]byte, error) {
		return nil, nil
	}
}
...
func makeTestDisconnectHandler() api.ConnHandler {
	return func(ctx *api.ConnContext, opcode uint32, body []byte) ([]byte, error) {
		ctx.Close()
		return nil, nil
	}
...
func TestEndToEnd_NegotiateAndCall(t *testing.T) {
	...
	server.Handle(echoOpcode, func(ctx *api.ConnContext, opcode uint32, body []byte) ([]byte, error) {
		callCount.Add(1)
	...
	client, err := api.Dial(ctx, transport.NewTCPTransport(), addr)
	if err != nil {
		t.Fatalf("dial: %v", err)
	}
	defer client.Close()
...
func TestEndToEnd_Notify(t *testing.T) {
	...
	server.Handle(notifyOpcode, func(ctx *api.ConnContext, opcode uint32, body []byte) ([]byte, error) {
		received <- body
		return nil, nil
	})
...
func TestEndToEnd_CustomNegotiator_Accept(t *testing.T) {
	...
	server.Handle(0x0100, func(ctx *api.ConnContext, opcode uint32, body []byte) ([]byte, error) {
		return body, nil
	})
