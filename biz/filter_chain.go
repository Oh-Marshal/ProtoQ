...
func (r *FilterChainRegister) AddFilter(f protoq.Filter) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *FilterChainRegister) DoFilter(ctx protoq.Context, completed protoq.FilterChainCompleted) error {
	r.mu.RLock()
	snapshot := make([]protoq.Filter, len(r.filters))
...
type continuation struct {
	snapshot  []protoq.Filter
	completed protoq.FilterChainCompleted
	index     int
}

// DoFilter 实现 protoq.FilterChain 接口。
//
...
func (c *continuation) DoFilter(ctx protoq.Context) error {
	// 连接已关闭则提前终止链
