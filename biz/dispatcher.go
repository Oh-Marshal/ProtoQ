...
type MessageDispatcher struct {
	...
	handlerRegistry map[uint32]protoq.MessageHandler
}
...
func NewMessageDispatcher() *MessageDispatcher {
	return &MessageDispatcher{
		filterChain:     &FilterChainRegister{},
		handlerRegistry: make(map[uint32]protoq.MessageHandler),
	}
}
...
// handler: 消息处理函数，签名 func(ctx protoq.Context) ([]byte, error)
func (d *MessageDispatcher) RegisterHandler(messageID uint32, handler protoq.MessageHandler) {
	d.mu.Lock()
	defer d.mu.Unlock()
...
func (d *MessageDispatcher) GetHandler(messageID uint32) protoq.MessageHandler {
	d.mu.RLock()
	defer d.mu.RUnlock()
...
func (d *MessageDispatcher) Dispatch(conn protoq.Connection, frame *protoq.Frame) (*MessageContext, error) {
	...
	completed := protoq.FilterChainCompleted(func(ctx protoq.Context) error {
		d.mu.RLock()
...
type EventDispatcher struct {
	mu              sync.RWMutex
	handlerRegistry map[string]protoq.EventHandler
}

// NewEventDispatcher 创建事件分发器。
func NewEventDispatcher() *EventDispatcher {
	return &EventDispatcher{
		handlerRegistry: make(map[string]protoq.EventHandler),
	}
}
...
// handler: 事件处理函数，签名 func(conn protoq.Connection, message interface{}) error
func (d *EventDispatcher) RegisterHandler(event string, handler protoq.EventHandler) {
	d.mu.Lock()
	defer d.mu.Unlock()
...
func (d *EventDispatcher) GetHandler(event string) protoq.EventHandler {
	d.mu.RLock()
	defer d.mu.RUnlock()
...
//  3. 未找到 → 返回错误
//  4. 找到 → 调用 handler(conn, message)
func (d *EventDispatcher) Dispatch(conn protoq.Connection, event string, message interface{}) error {
	if conn == nil || event == "" {
		return nil
	}
