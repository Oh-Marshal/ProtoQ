...
type ConnectionBridge struct {
	// conn 底层连接（*protoq.Conn，需要 frame 级读写能力）
	conn *protoq.Conn
...
//
// 参数：
//   - conn: 底层连接（*protoq.Conn，提供 Decode/WriteFrame 等传输层操作）
//   - beanReg: Bean 注册中心（已注册 Codec/Filter/Handler）
func NewConnectionBridge(conn *protoq.Conn, beanReg *BeanRegister) *ConnectionBridge {
	return &ConnectionBridge{
		conn:    conn,
		beanReg: beanReg,
		writeCh: make(chan *protoq.Frame, 64),
...
func (b *ConnectionBridge) handleFrame(frame *protoq.Frame) {
...
func (b *ConnectionBridge) decryptBody(frame *protoq.Frame) error {
	if len(frame.Body) == 0 {
		return nil
	}
...
func (b *ConnectionBridge) encryptBody(frame *protoq.Frame) error {
	if len(frame.Body) == 0 {
		return nil
	}
...
func (b *ConnectionBridge) WriteFrame(frame *protoq.Frame) {
	select {
...
func (b *ConnectionBridge) handleFatalError(requestFrame *protoq.Frame, err error) {
	// 尝试写错误响应帧
...
func (b *ConnectionBridge) writeErrorResponse(requestFrame *protoq.Frame, err error) {
	if requestFrame == nil || !requestFrame.RequiresAck() {
		return
	}

	errMsg := []byte(err.Error())
	resp := protoq.NewResponseFrame(requestFrame.Opcode, requestFrame.Seq, errMsg, requestFrame.Flags)
...
//   - response 是 *protoq.Frame → 直接写出
//   - response 是 []byte → 封装为响应帧写出
//   - response 是 nil → 不写回
func (b *ConnectionBridge) writeResponse(requestFrame *protoq.Frame, response interface{}) {
	...
	case []byte:
		// []byte → 封装为响应帧
		respFrame := protoq.NewResponseFrame(requestFrame.Opcode, requestFrame.Seq, resp, requestFrame.Flags)
	...
	case nil:
		...
		// 空 Body 响应帧
		respFrame := protoq.NewResponseFrame(requestFrame.Opcode, requestFrame.Seq, nil, requestFrame.Flags)
		b.WriteFrame(respFrame)
...
func (b *ConnectionBridge) Conn() *protoq.Conn {
	return b.conn
}
