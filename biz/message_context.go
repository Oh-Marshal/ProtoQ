...
// MessageContext 实现 protoq.Context 接口，承载单次请求的"连接 + 当前帧报文 + 响应载体"。
// 在解码 → 过滤链 → 业务分发的 Pipeline 中传递，不提供发送能力（发送由 Connection 负责）。
package biz

import "github.com/oh-marshal/protoq"

// MessageContext 是 protoq.Context 接口的具体实现。
//
...
// conn: 本次请求所属的连接
// frame: 解码后的请求帧
func NewMessageContext(conn protoq.Connection, frame *protoq.Frame) *MessageContext {
...
func (c *MessageContext) Connection() protoq.Connection {
	return c.conn
}
...
// Go 中 PacketData 对应 protoq.Frame 结构体。
func (c *MessageContext) Frame() *protoq.Frame {
	return c.frame
}
