...
### 2.1 变体 A：通用报文（有 Body，BODY_LEN=1）
...
### 2.2 变体 B：固定长度应答（无 Body，BODY_LEN=0）
...
变体 B 无 Length 字段和 Body，适用于简单应答（如 ACK-only 响应）。接收端通过 BODY_LEN 标志区分变体。
...
| Length | 0 或 2 | 大端序 uint16，BODY_LEN=1 时存在 |
...
┌───────┬───────┬───────┬───────┬───────┬───────┬───────┬───────┐
│  DIR  │ACK_REQ│BODY_LEN│  OP_LEN (2b) │  SEQ_LEN (2b) │CRC_LEN│
└───────┴───────┴───────┴───────┴───────┴───────┴───────┴───────┘
```
...
| bit6 | ACK_REQ | 0 | 无需应答 |
| | | 1 | 需要应答（此时 SEQ_LEN > 0） |
| bit5 | BODY_LEN | 0 | 变体 B，无 Length 和 Body |
| | | 1 | 变体 A，有 Length 字段 |
...
BODY_LEN=0  →  无 Body，无 Length    # 变体 B 无消息体
```
...
Flags 仅 1 字节（8 位），6 位用于 DIR/ACK_REQ/BODY_LEN/OP_LEN/SEQ_LEN，仅剩 1 位给 CRC_LEN。因此当前实现仅支持：
...
| `ErrBodyWithoutLength` | BODY_LEN=0 但有 Body |
